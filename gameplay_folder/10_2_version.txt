...
# --- 背景 ---
background = pygame.image.load("assets/BavkGroung.jpg").convert()
...
while running:
    ...
    for event in pygame.event.get():
        if event.type==pygame.QUIT:
            ...
            if event.key==pygame.K_SPACE:
                if player_money >= 10:
                    melee_walk_images = [pygame.image.load(f"assets/bird/melee_walk{i}.png") for i in range(1,8)]
                    melee_attack_images = [pygame.image.load(f"assets/bird/melee_attack{i}.png") for i in range(1,4)]
                    allies.add(Unit(
                        50, 300,
                        melee_walk_images,
                        melee_attack_images,
                        attack=5, attack_speed=0.8, hp=40,
                        range_type="melee", move_speed=1.0, attack_range=40
                    ))

                    player_money -=10
            elif event.key==pygame.K_m:
                if player_money >= 15:
                    allies.add(Unit(50, 300, range_image, 3, 0.5, 25,
                                    "ranged", move_speed=1.2, attack_range=150))
                    player_money -=15
    ...
    screen.blit(font.render("基地", True, BLACK), (WIDTH-270,20))
